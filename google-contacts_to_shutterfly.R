# google-contacts_to_shutterfly.R
# 
# Dan Spakowicz
...
colnames(tp) <- c("First Name", "Last Name", "Home Street", "Home Street 2", "Home City", "Home State", "Home Postal Code", "Home Country", "E-mail Address")

# Write output for uploading to TinyPrints
date <- Sys.Date() %>%
  as.Date(format = "%F")
write.csv(tp, file = paste(date, "holiday_shutterfly.csv", sep = "_"),
          row.names = FALSE, na = "")
